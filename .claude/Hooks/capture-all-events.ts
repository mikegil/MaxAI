...
function getCSTTimestamp(): string {
  const date = new Date();
  const cstDate = new Date(date.toLocaleString('en-US', { timeZone: process.env.TIME_ZONE || 'America/Chicago' }));
...
function getEventsFilePath(): string {
  const now = new Date();
  const pstDate = new Date(now.toLocaleString('en-US', { timeZone: process.env.TIME_ZONE || 'America/Chicago' }));
